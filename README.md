...
- **RAG 语义检索**：本地 Ollama 或云端 OpenAI 双引擎向量化，AI 在对话中先语义命中相关笔记再精读，告别「知识库一大就读不全」
...
| **Phase 2** | **v2.0** | **进化为“数字生命体”**<br>赋予 AI 长期记忆、个性化性格与主动服务能力，让它从工具变为真正的伙伴。<br>*（已落地：知识库 RAG 语义检索 + 双链知识图谱、可编程技能系统）* |  进行中 |
...
| 知识库检索 | sqlite-vec + numpy | 向量存储嵌入本地 SQLite，语义检索零外部服务 |
| 文档解析 | markitdown + python-docx + PyPDF2 + pandas | 图片、Word、PDF、Excel 等多格式内容提取 |
...
│   │   ├── components/     # ChatInput / MessageList / SettingsDrawer / ToolsDrawer / kb（知识库面板）
│   │   ├── composables/    # useChat / useModel / useFileUpload / useTheme 等组合式逻辑
│   │   ├── stores/         # chat / config / profiles / knowledge / skills / tools / mcp
│   │   ├── api/            # models.ts / knowledge.ts（RAG / 图谱 / 双链）
