...
| **Phase 2** | **v2.0** | **进化为“数字生命体”**<br>赋予 AI 长期记忆、个性化性格与主动服务能力，让它从工具变为真正的伙伴。<br>*（已落地：知识库 RAG 语义检索 + 双链知识图谱）* |  进行中 |
...
|       ├── kb_search.py    # 知识库语义检索工具（供 LLM 调用）
|       └── ...             # 其他通用基础工具
├── frontend/               # Vue 3 前端
│   ├── src/
│   │   ├── views/          # ChatWindow / KnowledgeView / KbGraphView（知识图谱）
│   │   ├── components/     # ChatWindow / SettingsDrawer / kb（知识库面板组件）
│   │   ├── stores/         # chat.ts / config.ts / profiles.ts / knowledge.ts
