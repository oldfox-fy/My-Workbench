...
async def _handle_chat(ws: WebSocket, data: dict, cancel_event: asyncio.Event,
    ...
    try:
        ...
        # ── 意图识别路由（方案 A+B 混合）──
        ws_max_steps = app_config.max_tool_steps
        ...
        try:
            ...
            if intent_router_enabled:
                ...
                intent_llm = service if (enable_tools and intent_llm_classify) else None
            ...
            if detected.intent != "standard":
                existing_names = {t["function"]["name"] for t in (tools or [])}
                for tool_name in detected.auto_inject_tools:
                    if tool_name not in existing_names:
                        ...
                        for t in inject_tool:
                            tools.append(t)
                ws_max_steps = detected.max_steps
                if tools:
                    tools.sort(key=lambda t: t.get("function", {}).get("name", ""))
                intent_notice = f"🧠 意图识别：{detected.intent}（{detected.reasoning}）— 步数 {detected.max_steps}"
        ...
        if intent_prompt_suffix:
            system_prompt = system_prompt + intent_prompt_suffix
